...
@interface LHHActionSheet()<UIGestureRecognizerDelegate>
{
    UIWindow *window;
}
...
-(void)loadUI{
    ...
    /*Space*/
    UILabel *spaceLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(ButtomView.bounds) - ItemHeight - Spacing, W, Spacing)];
    spaceLabel.backgroundColor = [UIColor colorWithRed:239.0f/255.0f green:239.0f/255.0f blue:241.0f/255.0f alpha:1];
    [_ButtomView addSubview:spaceLabel];
    ...
    for (NSString *Title in _Titles) {
        ...
        CGFloat hei = (ItemHeight * _Titles.count)+Spacing;
        ...
        [btn addTarget:self action:@selector(selectedButtons:) forControlEvents:UIControlEventTouchUpInside];
        [btn addTarget:self action:@selector(scaleToSmall:) forControlEvents:UIControlEventTouchDown | UIControlEventTouchDragEnter];
        [btn addTarget:self action:@selector(scaleAnimation:) forControlEvents:UIControlEventTouchUpInside];
